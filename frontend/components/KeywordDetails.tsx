  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
          ...
          <div className="relative w-full sm:w-64">
            <input
              ...
              className="w-full bg-page bg-opacity-90 border border-border rounded-xl py-2 pl-9 pr-8 text-xs text-primary placeholder-gray-500 focus:outline-none focus:border-sky-500/50 focus:ring-1 focus:ring-sky-500/30 transition-all duration-300"
            />
            <svg
            ...
            {searchTerm && (
              <button
                ...
                className="absolute right-2.5 top-2.5 text-muted hover:text-secondary"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-6 glow-card-teal">
          <div className="flex items-center justify-between mb-4 border-b border-border pb-2">
            <h4 className="text-xs font-bold text-teal-600 dark:text-teal-400 uppercase tracking-wider flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-2.5 max-h-80 overflow-y-auto scrollbar-thin pr-1">
            {keywords.matched
              ...
              .map((word) => {
                ...
                return (
                  <div
                    key={word}
                    className="text-xs font-semibold bg-teal-500/5 hover:bg-teal-500/10 border border-teal-500/20 hover:border-teal-400/40 text-teal-700 dark:text-teal-300 hover:text-teal-200 px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all duration-200 cursor-default"
                  >
                    ...
                    <span>{word}</span>
                    <span className="text-[9px] bg-surface text-teal-600 dark:text-teal-400 px-1.5 py-0.5 rounded-md font-mono">
                      {freq.resume}x
                    </span>
                  </div>
                );
            ...
            {keywords.matched.filter(k => k.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 && (
              <span className="text-xs text-muted italic py-2">No matched skills found.</span>
            )}
          </div>
        </div>
        ...
        <div className="glass-panel p-6 hover:shadow-[0_0_30px_rgba(244,63,94,0.1)] hover:border-rose-500/20 transition-all duration-300">
          ...
          <div className="flex flex-wrap gap-2.5 max-h-80 overflow-y-auto scrollbar-thin pr-1">
            {keywords.missing
              ...
              .map((word) => {
                ...
                return (
                  <div
                    key={word}
                    className="text-xs font-semibold bg-rose-500/5 hover:bg-rose-500/10 border border-rose-500/20 hover:border-rose-400/40 text-rose-700 dark:text-rose-300 hover:text-rose-200 px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all duration-200 cursor-default"
                  >
                    ...
                    <span>{word}</span>
                    <span className="text-[9px] bg-surface text-rose-600 dark:text-rose-400 px-1.5 py-0.5 rounded-md font-mono">
                      0/{freq.jd}x
                    </span>
                  </div>
                );
            ...
            {keywords.missing.filter(k => k.toLowerCase().includes(searchTerm.toLowerCase())).length === 0 && (
              <span className="text-xs text-muted italic py-2">No missing skills found. Great job!</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel p-6">
        <h4 className="text-xs font-bold text-primary uppercase tracking-wider mb-6 border-b border-border pb-2 flex items-center justify-between">
          <span>Keyword Frequency & Distribution Ratios</span>
          <span className="text-[10px] text-muted normal-case font-normal">Shows density ratio in Resume vs Job Description</span>
        </h4>

        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-border text-muted text-[10px] uppercase font-semibold tracking-wider">
            ...
            <tbody className="divide-y divide-white/5 text-xs">
              {currentKeywords.map((item) => {
                ...
                return (
                  <tr key={item.term} className="text-secondary hover:bg-surface transition-all duration-150">
                    <td className="py-3.5 font-semibold text-primary">{item.term}</td>
                    <td className="py-3.5 text-center font-mono">
                      <span className={`px-2.5 py-1 rounded-md font-bold text-xs ${
                        freq.resume > 0 ? 'bg-teal-500/10 text-teal-600 dark:text-teal-400 border border-teal-500/10' : 'bg-surface text-muted'
                      }`}>
                        {freq.resume}
                      </span>
                    </td>
                    <td className="py-3.5 text-center font-mono text-muted font-bold">{freq.jd}</td>
                    <td className="py-3.5">
                      <div className="flex flex-col gap-1 w-full max-w-xs">
                        {/* Resume vs JD Double Bar */}
                        <div className="flex items-center gap-1.5 text-[9px] text-muted">
                          <span className="w-8">Resume:</span>
                          <div className="w-full bg-page bg-opacity-90 rounded-full h-1.5 overflow-hidden border border-border">
                            <div
