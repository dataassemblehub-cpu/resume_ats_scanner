...
export default function SuggestionsSection({
    ...
    return (
      <div className="glass-panel p-10 flex flex-col items-center justify-center text-center gap-6 relative overflow-hidden min-h-[400px] border-l-4 border-l-amber-500">
        ...
        <div className="flex flex-col gap-2 max-w-md">
          <h3 className="text-lg font-bold text-primary">AI Suggestions Offline</h3>
  ...
  return (
    <div className="flex flex-col gap-6 relative">
      ...
      {/* Toast Alert overlay */}
      {toastMessage && (
        <div className="fixed bottom-6 right-6 bg-teal-600/95 text-primary font-bold text-xs px-4 py-2.5 rounded-xl backdrop-blur-md border border-teal-500/20 shadow-xl shadow-teal-500/10 animate-fade-in z-50 flex items-center gap-2 select-none">
      ...
      {isGenerating ? (
        <div className="glass-panel p-12 flex flex-col items-center justify-center text-center gap-6 min-h-[500px] relative overflow-hidden">
          ...
          <div className="flex flex-col gap-2 max-w-sm relative z-10">
            <h3 className="text-md font-bold text-primary tracking-wide animate-pulse">
        ...
        <div className="glass-panel p-12 flex flex-col items-center justify-center text-center gap-6 min-h-[450px] border-l-4 border-l-emerald-500 relative overflow-hidden">
          ...
          <div className="w-16 h-16 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shadow-lg shadow-emerald-500/10">
            <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <div className="flex flex-col gap-2 max-w-md relative z-10">
            <h3 className="text-lg font-extrabold text-primary tracking-tight">Outstanding Profile Match!</h3>
            <p className="text-xs text-muted leading-relaxed">
          ...
          <button
            ...
            className="px-4 py-2 border border-border bg-surface hover:bg-card border-border text-secondary text-xs font-semibold rounded-lg transition-all"
          >
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="glass-panel p-6 relative overflow-hidden glow-card-violet shrink-0 flex flex-col gap-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border pb-3">
                ...
                <button
                  ...
                  className="text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg bg-surface hover:bg-card border-border text-secondary border border-border hover:border-border transition-all flex items-center gap-1.5 self-start sm:self-center"
                >
                  {!canCopySuggestions ? (
              ...
              {/* Disclaimer reminder */}
              <div className="text-[10px] text-muted italic bg-white/[0.01] border border-border rounded-lg px-3 py-2 leading-relaxed">
              ...
              <p className="text-xs text-secondary leading-relaxed font-sans italic border-l-2 border-violet-500/35 pl-4 py-0.5">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="glass-panel p-4 flex flex-col items-center justify-center text-center gap-1 bg-rose-500/[0.02] border-rose-500/10">
              <span className="text-[9px] font-extrabold text-rose-600 dark:text-rose-400 uppercase tracking-widest">🔴 High Impact</span>
              <span className="text-xl font-bold text-primary font-mono">{highCount}</span>
              <span className="text-[8px] text-muted">Critical gaps to resolve</span>
            </div>
            ...
            <div className="glass-panel p-4 flex flex-col items-center justify-center text-center gap-1 bg-amber-500/[0.02] border-amber-500/10">
              <span className="text-[9px] font-extrabold text-amber-600 dark:text-amber-400 uppercase tracking-widest">🟡 Medium Impact</span>
              <span className="text-xl font-bold text-primary font-mono">{mediumCount}</span>
              <span className="text-[8px] text-muted">Optimizations to apply</span>
            </div>
            ...
            <div className="glass-panel p-4 flex flex-col items-center justify-center text-center gap-1 bg-emerald-500/[0.02] border-emerald-500/10">
              <span className="text-[9px] font-extrabold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">🟢 Nice to Have</span>
              <span className="text-xl font-bold text-primary font-mono">{lowCount}</span>
              <span className="text-[8px] text-muted">Style tips to review</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Strengths Card */}
            <div className="glass-panel p-5 border-l-4 border-l-teal-500/80 relative glow-card-teal ">
              <h3 className="text-xs font-bold text-teal-600 dark:text-teal-400 uppercase tracking-wider mb-4 flex items-center justify-between">
                <span>Core Match Strengths</span>
                <span className="text-[8px] text-muted">Positive indicators</span>
              </h3>
              {strengths.length > 0 ? (
                <ul className="flex flex-col gap-3">
                  {strengths.map((strength, index) => (
                    <li key={index} className="text-xs text-secondary flex items-start gap-2.5 leading-relaxed">
                      <svg className="w-4 h-4 text-teal-600 dark:text-teal-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                ...
                <p className="text-xs text-muted italic">No significant strengths reported.</p>
              )}
            </div>

            {/* Gaps / Weaknesses Card */}
            <div className="glass-panel p-5 border-l-4 border-l-rose-500/80 relative hover:shadow-[0_0_30px_rgba(244,63,94,0.1)] transition-all ">
              <h3 className="text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider mb-4 flex items-center justify-between">
              ...
              {weaknesses.length > 0 ? (
                <ul className="flex flex-col gap-3">
                  {weaknesses.map((weakness, index) => (
                    <li key={index} className="text-xs text-secondary flex items-start gap-2.5 leading-relaxed">
                      <svg className="w-4 h-4 text-rose-600 dark:text-rose-400 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                      </svg>
                      <span>{weakness}</span>
                    </li>
          ...
          {missing_skills.length > 0 && (
            <div className="glass-panel p-5 border border-border relative glow-card-sky">
              <h3 className="text-xs font-bold text-sky-600 dark:text-sky-400 uppercase tracking-wider flex items-center gap-1.5 mb-1">
              ...
              <p className="text-[9px] text-muted mb-3.5 leading-relaxed">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all flex items-center gap-1.5 select-none ${
                        isCopied
                          ...
                          : 'bg-sky-500/5 hover:bg-sky-500/15 border-sky-500/20 hover:border-sky-400/40 text-sky-700 dark:text-sky-300 cursor-pointer active:scale-95'
                      }`}
                    >
                      <span>{skill}</span>
                      {isCopied ? (
                        <svg className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                        </svg>
                      ) : (
                        <svg className="w-3 h-3 text-sky-500/50 dark:text-sky-400/50 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
                        </svg>
                      )}
                    </button>
          ...
          {suggested_bullet_points.length > 0 && (
            <div className="glass-panel p-5 relative">
              ...
              <h3 className="text-xs font-bold text-teal-600 dark:text-teal-400 uppercase tracking-wider mb-2 flex items-center justify-between">
                ...
                <span className="text-[8px] text-amber-600 dark:text-amber-400 font-semibold font-mono">🟡 Medium Impact ({suggested_bullet_points.length})</span>
              </h3>
              <p className="text-[9px] text-muted mb-4 leading-relaxed">
              ...
              <div className="flex flex-col gap-4">
                {suggested_bullet_points.map((rawBullet, index) => {
                  ...
                  return (
                    <div key={index} className="flex flex-col border border-border bg-sidebar rounded-xl overflow-hidden">
                      {/* Header bar */}
                      <div className="flex items-center justify-between px-3.5 py-2 border-b border-border bg-surface text-[9px] font-bold">
                        <span className="text-teal-600 dark:text-teal-400">Optimization Bullet #{index + 1}</span>
                        ...
                        <button
                          ...
                          className="text-sky-600 dark:text-sky-400 hover:text-sky-700 dark:hover:text-sky-300 uppercase tracking-wider flex items-center gap-1 transition-colors"
                        >
                          {isCopied ? (
                            <>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border text-xs">
                        {parsed.before ? (
                          <div className="p-3.5 bg-rose-500/[0.01]">
                            <span className="text-[8px] font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider block mb-1">Before (Original):</span>
                            <p className="line-through decoration-rose-500/25 leading-relaxed text-muted font-sans">{parsed.before}</p>
                          </div>
                        ) : (
                          <div className="p-3.5 bg-black/5 dark:bg-white/5">
                            <span className="text-[8px] font-bold text-muted uppercase tracking-wider block mb-1">Base Context:</span>
                            <p className="italic leading-relaxed text-muted font-sans">Reference experience context parsed from original resume.</p>
                          </div>
                        )}
                        <div className="p-3.5 bg-teal-500/[0.02]">
                          <span className="text-[8px] font-bold text-teal-600 dark:text-teal-400 uppercase tracking-wider block mb-1">After (Suggested):</span>
                          <p className="text-teal-700 dark:text-teal-300 font-mono leading-relaxed">{parsed.after}</p>
                        </div>
                      </div>
                    </div>
                  );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* ATS Advice */}
            <div className="glass-panel p-5 glow-card-violet ">
              <h3 className="text-xs font-bold text-violet-400 uppercase tracking-wider mb-4 flex items-center justify-between">
                ...
                <span className="text-[8px] text-muted">Machine compatibility</span>
              </h3>
              <ul className="flex flex-col gap-3">
                ...
                {ats_improvements.length === 0 && ats_recommendations.length === 0 && (
                  <li className="text-xs text-muted italic">No structural compatibility items needed.</li>
                )}
              </ul>
            </div>

            {/* Recruiter Advice */}
            <div className="glass-panel p-5 glow-card-sky ">
              <h3 className="text-xs font-bold text-sky-600 dark:text-sky-400 uppercase tracking-wider mb-4 flex items-center justify-between">
                <span>Human Recruiter Advice</span>
                <span className="text-[8px] text-muted">Readability & impact</span>
              </h3>
              <ul className="flex flex-col gap-3">
                ...
                {recruiter_improvements.length === 0 && resume_improvements.length === 0 && (
                  <li className="text-xs text-muted italic">No layout or readability recommendations.</li>
                )}
              </ul>
            </div>
          ...
          <div className="glass-panel p-6 border border-border bg-gradient-to-r from-violet-500/5 to-sky-500/5 flex flex-col sm:flex-row sm:items-center justify-between gap-6 mt-2">
            ...
            <button
              ...
              className={`px-5 py-2.5 text-xs font-bold uppercase tracking-wider rounded-lg transition-all shadow-md flex items-center gap-1.5 self-start sm:self-auto ${
                !canGenerateAI 
                  ? 'bg-neutral-800/40 border border-neutral-700/30 text-muted cursor-not-allowed opacity-50' 
                  : 'bg-surface border border-border hover:bg-card border-border text-secondary cursor-pointer active:scale-95'
              }`}
            >
              {!canGenerateAI ? (
